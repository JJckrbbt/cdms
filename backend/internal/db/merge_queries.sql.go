...
const deactivateChargebacksBySource = `-- name: DeactivateChargebacksBySource :exec
...
func (q *Queries) DeactivateChargebacksBySource(ctx context.Context, reportingSource ChargebackReportingSource) error {
	_, err := q.db.Exec(ctx, deactivateChargebacksBySource, reportingSource)
	return err
}

const deactivateNonIpacsBySource = `-- name: DeactivateNonIpacsBySource :exec
UPDATE "nonipac" SET is_active = false WHERE reporting_source = $1
`

func (q *Queries) DeactivateNonIpacsBySource(ctx context.Context, reportingSource NonipacReportingSource) error {
	_, err := q.db.Exec(ctx, deactivateNonIpacsBySource, reportingSource)
	return err
}

const getChargebackSourcesByBDDocNums = `-- name: GetChargebackSourcesByBDDocNums :many
SELECT bd_doc_num, al_num, reporting_source FROM chargeback
WHERE bd_doc_num = ANY($1::text[])
`

type GetChargebackSourcesByBDDocNumsRow struct {
...
func (q *Queries) GetChargebackSourcesByBDDocNums(ctx context.Context, dollar_1 []string) ([]GetChargebackSourcesByBDDocNumsRow, error) {
	rows, err := q.db.Query(ctx, getChargebackSourcesByBDDocNums, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetChargebackSourcesByBDDocNumsRow
	for rows.Next() {
		var i GetChargebackSourcesByBDDocNumsRow
		if err := rows.Scan(&i.BdDocNum, &i.AlNum, &i.ReportingSource); err != nil {
			return nil, err
		}
...
const upsertAgencyBureaus = `-- name: UpsertAgencyBureaus :execrows
...
func (q *Queries) UpsertAgencyBureaus(ctx context.Context) (int64, error) {
	result, err := q.db.Exec(ctx, upsertAgencyBureaus)
	if err != nil {
		return 0, err
	}
...
const upsertChargebacks = `-- name: UpsertChargebacks :execrows
...
func (q *Queries) UpsertChargebacks(ctx context.Context) (int64, error) {
	result, err := q.db.Exec(ctx, upsertChargebacks)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
...
const upsertNonIpacs = `-- name: UpsertNonIpacs :execrows
INSERT INTO "nonipac" (
...
func (q *Queries) UpsertNonIpacs(ctx context.Context) (int64, error) {
	result, err := q.db.Exec(ctx, upsertNonIpacs)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
